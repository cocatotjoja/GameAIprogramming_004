...
int Pathfinder::ShouldAdd(Node(&map1)[100][100], int x, int y, int corner, int width, int height)
{
	...
	if (x < width - 1 && x > 0 && y < height - 1 && y > 0)
	{
		...
		{
		case 1:
			if (map1[x + 1][y].BlockedORFogged() || map1[x][y - 1].BlockedORFogged())
			{
				return 0;
			}
			break;
...
int Pathfinder::ScoutShouldAdd(Node(&map1)[100][100], int x, int y, int corner, int width, int height)
{
	//Check if within map
	if (x < width - 1 && x > 0 && y < height - 1 && y > 0)
	{
...
void Pathfinder::ScoutAddNode(Node(&map1)[100][100], Vector2 parentID, int parentXplus, int parentYplus, float addValue, int corner, float parentValue, int width, int height, Vector2 goal)
{
	//std::cout << "Testing Node: " << parentXplus << " , " << parentYplus << std::endl;
	switch (ScoutShouldAdd(map1, parentXplus, parentYplus, corner, width, height))
	{
	case 1:
		// Update Value, Parent, and state
		map1[parentXplus][parentYplus].SetValues(parentValue + addValue, goal);
...
{
	if ((int)start.x < 0 || (int)start.x >= 100 ||
		(int)start.y < 0 || (int)start.y >= 100)
	{
	...
	while (!found)
	{
		...
		else
		{
			...
			int x = static_cast<int>(parentID.x);
			int y = static_cast<int>(parentID.y);
...
void Pathfinder::ScoutPath(Node(&map1)[100][100], Vector2 start, Vector2 goal, std::stack<Vector2>& returnPath)
{
	if ((int)start.x < 0 || (int)start.x >= 100 ||
		(int)start.y < 0 || (int)start.y >= 100)
	{
	...
	while (!found)
	{
		...
		else
		{
			...
			// ADD Adjacent nodes
			//map1[(int)parentID.x + 1][(int)parentID.y];
			ScoutAddNode(map1, parentID, (int)parentID.x + 1, (int)parentID.y, 1.0f, 0, parentValue, width, height, goal);
			//map1[(int)parentID.x - 1][(int)parentID.y];
			ScoutAddNode(map1, parentID, (int)parentID.x - 1, (int)parentID.y, 1.0f, 0, parentValue, width, height, goal);
			//map1[(int)parentID.x][(int)parentID.y + 1];
			ScoutAddNode(map1, parentID, (int)parentID.x, (int)parentID.y + 1, 1.0f, 0, parentValue, width, height, goal);
			//map1[(int)parentID.x][(int)parentID.y - 1];
			ScoutAddNode(map1, parentID, (int)parentID.x, (int)parentID.y - 1, 1.0f, 0, parentValue, width, height, goal);

			// ADD Diagonal Nodes
			//map1[(int)parentID.x - 1][(int)parentID.y + 1];
			ScoutAddNode(map1, parentID, (int)parentID.x - 1, (int)parentID.y + 1, 1.4f, 1, parentValue, width, height, goal);
			//map1[(int)parentID.x + 1][(int)parentID.y + 1];
			ScoutAddNode(map1, parentID, (int)parentID.x + 1, (int)parentID.y + 1, 1.4f, 2, parentValue, width, height, goal);
			//map1[(int)parentID.x - 1][(int)parentID.y - 1];
			ScoutAddNode(map1, parentID, (int)parentID.x - 1, (int)parentID.y - 1, 1.4f, 3, parentValue, width, height, goal);
			//map1[(int)parentID.x + 1][(int)parentID.y - 1];
			ScoutAddNode(map1, parentID, (int)parentID.x + 1, (int)parentID.y - 1, 1.4f, 4, parentValue, width, height, goal);
